...
def change_players_positions(shift):
    import operator
    from app.player_class import Player
    player_list = Player.player_list #shift positions for each player
    number_players = len(player_list)
    for player in player_list:
        player.position = (player.position + shift) % number_players 
    player_list.sort(key=operator.attrgetter('position')) #sort players by their updated position
